...
//
// Unlike the mock-LLM suite, this drives WorkflowRuntime against the REAL mty
...
// the live router + a real key in ~/.config/mtycoder/mtycoder.json). Run with:
...
// The real mty provider, read verbatim from the user's config (key included).
function realConfig() {
  const home = path.join(os.homedir(), ".config", "mtycoder", "mtycoder.json")
  const full = JSON.parse(readFileSync(home, "utf8"))
  const mty = full.provider?.mty
  if (!mty?.options?.apiKey) throw new Error("no mty provider/key in " + home)
  return { provider: { mty } }
}

const realRef = { providerID: ProviderID.make("mty"), modelID: ModelID.make("mty-v2.5-pro") }
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .mtycoder/workflows.
            const wfDir = path.join(dir, ".mtycoder", "workflows")
