...
describe("journalKey", () => {
  ...
  test("opts key order does not change the hash", () => {
    const a = journalKey("p", { agentType: "g", model: { providerID: "mty", modelID: "m" } }, 0)
    const b = journalKey("p", { model: { modelID: "m", providerID: "mty" }, agentType: "g" }, 0)
    expect(a).toBe(b)
  })
  ...
  test("a string model ref hashes by its value, not its resolution", () => {
    ...
    expect(journalKey("p", { agentType: "g", model: "mimo/mty-v2.5-pro" }, 0)).not.toBe(lite)
