...
const managedConfigDir = process.env.MTYCODER_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "mtycoder.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "mtycoder.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "mtycoder.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["MTYCODER_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:MTYCODER_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["MTYCODER_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["MTYCODER_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "mtycoder.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .mtycoder directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mtycoder")
...
test("loads agents from .mtycoder/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mtycoder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .mtycoder/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mtycoder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .mtycoder/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mtycoder")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("mtycoder command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".mtycoder", "command", "dup.md"),
        `---
description: mtycoder version
---
from mtycoder`,
      )
    },
...
test("does not try to install dependencies in read-only MTYCODER_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.MTYCODER_CONFIG_DIR
  process.env.MTYCODER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MTYCODER_CONFIG_DIR
    else process.env.MTYCODER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MTYCODER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .mtycoder config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".mtycoder")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .mtycoder config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "mtycoder.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mtycoder config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "mtycoder.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".mtycoder")
...
// Note: preload.ts sets MTYCODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .mtycoder config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mtycoder directory config enables it
      const opencodeDir = path.join(dir, ".mtycoder")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "mtycoder.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "mtycoder.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.mtycoder/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.mtycoder/plugin/demo.ts", "file:///project/.mtycoder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.mtycoder/plugin/demo.ts"])
  })
...
describe("MTYCODER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MTYCODER_DISABLE_PROJECT_CONFIG"]
    process.env["MTYCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MTYCODER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MTYCODER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .mtycoder/ directories when flag is set", async () => {
    const originalEnv = process.env["MTYCODER_DISABLE_PROJECT_CONFIG"]
    process.env["MTYCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .mtycoder directory with a command
          const opencodeDir = path.join(dir, ".mtycoder", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .mtycoder should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MTYCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MTYCODER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MTYCODER_CONFIG_DIR"]
      process.env["MTYCODER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MTYCODER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["MTYCODER_CONFIG_DIR"]
      } else {
        process.env["MTYCODER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("MTYCODER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MTYCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MTYCODER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MTYCODER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MTYCODER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from MTYCODER_CONFIG_DIR, not project
...
describe("MTYCODER_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in MTYCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MTYCODER_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["MTYCODER_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["MTYCODER_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["MTYCODER_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in MTYCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MTYCODER_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.mtycoder.managed.test",
          PayloadType: "ai.mtycoder.managed",
