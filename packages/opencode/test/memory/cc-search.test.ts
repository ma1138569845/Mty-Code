...
describe("memory.search with cc scope", () => {
  it.live("ranks CC and mty hits in one result list", () =>
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(path.join(mtyRoot, "global", "m.md"), "distinctivetoken from mimo"),
          )
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() => fs.rm(mtyRoot, { recursive: true, force: true }))
          yield* Effect.promise(() => fs.mkdir(path.join(mtyRoot, "global"), { recursive: true }))
          yield* Effect.promise(() => fs.writeFile(path.join(mtyRoot, "global", "m.md"), "tokenz"))
