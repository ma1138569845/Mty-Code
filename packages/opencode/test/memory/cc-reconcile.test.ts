  ...
  itLive.live("indexes CC files under scope='cc' with frontmatter type", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const mtyRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(mtyRoot, { recursive: true, force: true }))
...
Body about cache.`
        ...
        yield* Effect.promise(() => reconcileMemory({ mty: mtyRoot, cc: ccBase }))
  ...
  itLive.live("union prune: deleting one CC file leaves mty rows untouched", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const memory = yield* Memory.Service
        const mtyRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(mtyRoot, { recursive: true, force: true }))
        yield* Effect.promise(() => fs.mkdir(path.join(mtyRoot, "global"), { recursive: true }))
        yield* Effect.promise(() => fs.writeFile(path.join(mtyRoot, "global", "m.md"), "mty body"))
...
cc body`,
        ...
        expect(rows.length).toBe(1)
        expect(rows[0].scope).toBe("global") // mty row preserved
      }),
    ),
  )
...
body`,
        ...
        yield* Effect.promise(() => reconcileMemory({ mty: mtyRoot }))
        expect(
          Database.use((db) =>
            db.select().from(MemoryFtsTable).where(eq(MemoryFtsTable.scope, "cc")).all(),
          ).length,
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.promise(() =>
          reconcileMemory({ mty: mtyRoot, cc: "/definitely/not/a/real/path/abc123" }),
        )
