...
const envPath = process.env.MTYCODER_BIN_PATH
if (envPath) {
  run(envPath)
}
...
//
const cached = path.join(scriptDir, ".mtycoder")
...
const scope = "@mty-coder/"
const base = scope + "mtycoder-" + platform + "-" + arch
const binary = platform === "windows" ? "mty.exe" : "mty"
