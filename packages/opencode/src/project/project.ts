...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".mtycoder-project-id")
  const idFile = nodePath.join(mainGit, "mtycoder-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .mtycoder-project-id
  ...
  if (!existing.includes(".mtycoder-project-id")) {
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.MTYCODER_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        // turn, which has shipped as a bug for users opening mtycoder in $HOME.
      ...
      if (Flag.MTYCODER_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
