...
export const dict = {
  ...
  "tui.home.agreement.prefix": "MtyCoder をご利用いただくことで、",
  ...
  "tui.tips.share": "{highlight}/share{/highlight} を実行すると mtycoder.example.com に会話の公開リンクを作成します",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}mtycoder.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/mtycoder/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "MtyCoder は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.mtycoder/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.mtycoder/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "MtyCoder は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "MtyCoder はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.mtycoder/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.mtycoder/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて MtyCoder が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}mty run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}mty --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}mty run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}mty serve{/highlight} を実行するとヘッドレスで MtyCoder API にアクセスできます",
  "tui.tips.attach_server": "{highlight}mty run --attach{/highlight} で起動中のサーバに接続します",
  "tui.tips.upgrade": "{highlight}mty upgrade{/highlight} で最新バージョンに更新します",
  "tui.tips.auth_list": "{highlight}mty auth list{/highlight} で設定済みプロバイダを確認します",
  "tui.tips.agent_create": "{highlight}mty agent create{/highlight} でガイド付きエージェント作成を行います",
  "tui.tips.github_trigger": "GitHub の issue/PR で {highlight}/opencode{/highlight} を入力すると AI を起動します",
  "tui.tips.github_install": "{highlight}mty github install{/highlight} で GitHub workflow を設定します",
  ...
  "tui.tips.theme_files": "{highlight}.mtycoder/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}mty debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.tips.docker":
    "{highlight}docker run -it --rm ghcr.io/matyianyuan/MtyCoder{/highlight} でコンテナ版を実行できます",
  "tui.tips.zen": "MtyCoder と組み合わせて {highlight}/connect{/highlight} で厳選されたモデルを使えます",
  ...
  "tui.toast.update_available.success": "MtyCoder v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "tui.voice.error.no_auth": "/connect で MtyCoder アカウントに接続するか、voice.asr_model で別のプロバイダーを設定してください",
  ...
  "tui.dialog.login.mty_free.desc": "ログイン不要の匿名無料チャネル",
  "tui.dialog.login.mty_free.success": "MtyCoder Auto (free) の準備完了 — デフォルトモデルを mty/mty-auto に設定しました",
  "tui.dialog.login.mty_free.unavailable": "MtyCoder Auto (free) プロバイダーが読み込まれていません",
  "tui.dialog.login.flow.title": "MtyCoder ログイン",
  ...
  "cli.providers.mty.recommended_hint": "推奨",
  "cli.providers.mty_free.hint": "匿名無料チャネル / mty-auto",
  "cli.providers.mty_free.verifying": "MtyCoder Auto (free) チャネルを検証中...",
  "cli.providers.mty_free.ready": "MtyCoder Auto (free) チャネル準備完了",
  "cli.providers.mty_free.failed": "MtyCoder Auto (free) の自己診断に失敗",
  "cli.providers.mty_free.default_set": "デフォルトモデルを mty/mty-auto に設定（1M コンテキスト、無料）",
  "cli.providers.mty_free.usage_hint":
    "ログイン不要 — そのまま mty を実行できます。有料/上位モデルを利用する場合は MiMo ブラウザログインを選択してください。",
  "cli.providers.mty_login.decrypt_retry": "復号に失敗しました、再試行してください（残り {remaining} 回）",
  "cli.providers.mty_login.decrypt_exhausted": "復号に失敗しました、最大再試行回数に達しました",
  ...
  "trust.capabilities": "MtyCoder はここでファイルの読み取り、編集、実行が可能になります。",
