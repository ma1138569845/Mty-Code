...
import type { GlobalEvent } from "@mty-coder/sdk/v2"
import type { EventSource } from "./context/sdk"
import { win32DisableProcessedInput, win32InstallCtrlCGuard } from "./win32"
import { writeHeapSnapshot } from "v8"
import { TuiConfig } from "./config/tui"
import { MTYCODER_PROCESS_ROLE, MTYCODER_RUN_ID, ensureRunID, sanitizedProcessEnv } from "@/util/mty-process"
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start mtycoder tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start mtycoder in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [MTYCODER_PROCESS_ROLE]: "worker",
        [MTYCODER_RUN_ID]: ensureRunID(),
      })
