...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run mtycoder",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for mtycoder session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found mtycoder session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["mty", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting mtycoder...")
        UI.println()

        const mtyArgs = sessionId ? ["-s", sessionId] : []
        const mtyProcess = Process.spawn(["mty", ...mtyArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await mtyProcess.exited
        if (code !== 0) throw new Error(`mty exited with code ${code}`)
      },
    })
  },
})
