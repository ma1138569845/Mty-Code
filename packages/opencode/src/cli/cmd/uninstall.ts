...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall mtycoder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall MtyCoder")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @mty-coder/cli",
      pnpm: "pnpm uninstall -g @mty-coder/cli",
      bun: "bun remove -g @mty-coder/cli",
      // TODO(mtycoder): uncomment when published to these channels
      // brew: "brew uninstall mtycoder",
      // choco: "choco uninstall mtycoder",
      // scoop: "scoop uninstall mtycoder",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using MtyCoder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# mtycoder") || content.includes(".mtycoder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".mtycoder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".mtycoder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".mtycoder"))
    ) {
      continue
    }
