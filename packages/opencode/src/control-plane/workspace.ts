...
import { Slug } from "@mty-coder/shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    MTYCODER_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    MTYCODER_WORKSPACE_ID: config.id,
    MTYCODER_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.MTYCODER_EXPERIMENTAL_WORKSPACES) return
