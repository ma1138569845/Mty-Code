...
import { createOpencodeClient } from "@mty-coder/sdk"
import { Flag } from "../flag/flag"
import { CodexAuthPlugin } from "./codex"
import { MtyAuthPlugin, AnthropicProxyPlugin } from "./mty"
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createOpencodeClient({
          ...
          headers: Flag.MTYCODER_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.MTYCODER_SERVER_USERNAME ?? "mtycoder"}:${Flag.MTYCODER_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        // "mty-auto" channel). That directory is NOT part of the open-source
        ...
        const plugins = Flag.MTYCODER_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.MTYCODER_PURE && cfg.plugin_origins?.length) {
