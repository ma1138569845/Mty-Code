...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in mtycoder config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
const InfoSchema = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for mty serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://mtycoder.example.com/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://mtycoder.example.com/docs/agents" }),
  ...
  memory: Schema.optional(
    Schema.Struct({
      cc_index: Schema.optional(Schema.Boolean).annotate({
        description:
          "Index Claude Code memory (~/.claude/projects/<slug>/memory) and expose under scope='cc'. Default: false. Note: when enabled, every mtycoder agent (build/explore/subagents) can search these memories via the builtin `memory` tool — including CC's `type: user` (your role/preferences) and `type: feedback` (your guidance) categories. CC originally writes them for future CC sessions; flipping this on widens the consumer set to mtycoder agents on the same machine. Leave disabled (default) if you don't want personal context recallable from a prompt-injection-vulnerable agent.",
      }),
    }),
  ),
  ...
  voice: Schema.optional(
    Schema.Struct({
      asr_model: Schema.optional(ConfigModelID).annotate({
        description:
          "Model to use for voice ASR transcription in provider/model format. Defaults to mty-v2.5-asr.",
      }),
      control_model: Schema.optional(ConfigModelID).annotate({
        description:
          "Model to use for voice control (multimodal) in provider/model format. Defaults to mty-v2.5.",
      }),
    }),
...
function globalConfigFile() {
  const candidates = ["mtycoder.jsonc", "mtycoder.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://mtycoder.example.com/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "mtycoder.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "mtycoder.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://mtycoder.example.com/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "MTYCODER_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://mtycoder.example.com/config.json"
        ...
        if (Flag.MTYCODER_CONFIG) {
          yield* merge(Flag.MTYCODER_CONFIG, yield* loadFile(Flag.MTYCODER_CONFIG))
          log.debug("loaded custom config", { path: Flag.MTYCODER_CONFIG })
        }

        if (!Flag.MTYCODER_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("mtycoder", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.MTYCODER_CONFIG_DIR) {
          log.debug("loading config from MTYCODER_CONFIG_DIR", { path: Flag.MTYCODER_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .mtycoder commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".mtycoder") || dir === Flag.MTYCODER_CONFIG_DIR) {
            for (const file of ["mtycoder.json", "mtycoder.jsonc"]) {
          ...
          // Auto-discovered plugins under `.mtycoder/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.MTYCODER_CONFIG_CONTENT) {
          const source = "MTYCODER_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.MTYCODER_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from MTYCODER_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["MTYCODER_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("MTYCODER_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (!Flag.MTYCODER_DISABLE_CLAUDE_CODE_MCP) {
        ...
        if (Flag.MTYCODER_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.MTYCODER_PERMISSION))
        }
        ...
        if (Flag.MTYCODER_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.MTYCODER_DISABLE_PRUNE) {
