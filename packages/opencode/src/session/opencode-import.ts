  ...
  try {
    ...
    for (const sess of sessions) {
      ...
      try {
        ...
        // If this import previously created a mtycoder session that the user has
        ...
        // session backward past mtycoder-native activity (cc/codex parity).
        let existingUpdated: number | undefined
        if (existing) {
          const mtySess = Database.use((db) =>
          ...
          if (!mtySess) {
            ...
            existingUpdated = mtySess.updated
          }
        }
