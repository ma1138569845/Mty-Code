...
const Headers = Schema.Struct({
  ...
  "x-mtycoder-directory": Schema.optional(Schema.String),
})
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.MTYCODER_SERVER_PASSWORD) return yield* effect

        const user = Flag.MTYCODER_SERVER_USERNAME ?? "mtycoder"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.MTYCODER_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-mtycoder-directory"] || process.cwd()
