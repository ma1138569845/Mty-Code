...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@mty-coder/plugin"
import z from "zod"
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.MTYCODER_CLIENT) || Flag.MTYCODER_ENABLE_QUESTION_TOOL

        // Eagerly initialize all tools (simpler and more reliable)
        ...
        return {
          ...
          builtin: [
            ...
            tool.patch,
            tool.changedir,
            ...(Flag.MTYCODER_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            tool.plan,
            tool.memory,
            tool.history,
            tool.task,
            ...(Flag.MTYCODER_EXPERIMENTAL_WORKFLOW_TOOL ? [tool.workflow] : []),
          ],
          actor: tool.actor,
          read: tool.read,
        }
        ...
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          if (tool.id === WebSearchTool.id) {
            return (
              ...
              input.providerID === "mtycoder" ||
              Flag.MTYCODER_ENABLE_EXA
            )
          }
          return input.providerID === ProviderID.opencode || Flag.MTYCODER_ENABLE_EXA
        }
