...
import * as MtyWebsearch from "./mty"
import { Auth } from "@/auth"
import { Provider } from "@/provider"
import DESCRIPTION from "./websearch.txt"

const WEBFETCH_FALLBACK =
  "Web search unavailable. Use `webfetch` with a relevant URL instead, or enable the Web Search plugin at https://platform.mtycoder.example.com/console/plugin."
    ...
    return {
      ...
      execute: (params: z.infer<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const result =
            model?.providerID === "mtycoder"
              ? yield* Effect.catchCause(
                  Effect.gen(function* () {
                    const info = yield* auth.get("mtycoder")
                    if (!info || info.type !== "api") return undefined
                    return yield* MtyWebsearch.call(
                      http,
                      model.api.url,
                      info.key,
                      params.query,
