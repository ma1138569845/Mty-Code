...
const PACKAGE_NAME = "@mty-coder/cli"
...
export const USER_AGENT = `mtycoder/${InstallationChannel}/${InstallationVersion}/${Flag.MTYCODER_CLIENT}`
...
// TODO(mtycoder): uncomment when corresponding channels are supported
...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | ChildProcessSpawner.ChildProcessSpawner> =
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // TODO(mtycoder): uncomment when mtycoder is published to homebrew
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          const response = yield* httpOk.execute(HttpClientRequest.get("https://mtycoder.example.com/install"))
      ...
      const methodImpl = Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".mtycoder", "bin"))) return "curl" as Method
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          // TODO(mtycoder): uncomment when mtycoder is published to these channels
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "curl") {
          const headers = yield* text([
            "curl",
            "-sI",
            "https://github.com/matyianyuan/MtyCoder/releases/latest",
          ])
        ...
        // TODO(mtycoder): uncomment when mtycoder is published to chocolatey
        ...
        // TODO(mtycoder): uncomment when mtycoder is published to scoop
        ...
        // TODO(mtycoder): uncomment when mtycoder has github releases
        // const response = yield* httpOk.execute(
        //   HttpClientRequest.get("https://api.github.com/repos/matyianyuan/MtyCoder/releases/latest").pipe(
        //     HttpClientRequest.acceptJson,
        //   ),
        // )
      ...
      const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        if (!result || result.code !== 0) {
          // TODO(mtycoder): restore choco-specific error when choco channel is supported
