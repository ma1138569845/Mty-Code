...
You can extend your own capabilities by writing files to `.mtycoder/`. Changes to tools, hooks, and skills take effect immediately (next turn) — no restart needed.

## Creating Tools

Write to `.mtycoder/tools/<name>.ts`:

```ts
import { tool } from "@mty-coder/plugin"
...
Write to `.mtycoder/hooks/<name>.ts` — export a Hooks object:
...
```ts
// .mtycoder/tools/bash.ts — overrides built-in bash
import { tool } from "@mty-coder/plugin"
...
Write to `.mtycoder/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.mtycoder/tools/*.ts` | next turn |
| Hooks | `.mtycoder/hooks/*.ts` | next turn |
| Skills | `.mtycoder/skills/*/SKILL.md` | next turn |
| TUI | `.mtycoder/tui/*.tsx` | restart |
