<h1 align="center">MtyCoder</h1>

<p align="center">
  <img src="assets/readme/mtycoder-banner.png" alt="MtyCoder" width="700">
</p>

<p align="center"><strong>MtyCoder: Where Models and Agents Co-Evolve</strong></p>
...
<p align="center">
  <a href="https://mtycoder.example.com/en/mtycoder">Website</a> | <a href="https://mtycoder.example.com/en/blog/mty-code-long-horizon">Blog</a>
</p>

---

MtyCoder is a terminal-native AI coding assistant. It can read and write code, run commands, manage Git, and use a persistent memory system to keep a deep understanding of your project across sessions while continuously improving itself.

MtyCoder Auto is built in as a free-for-limited-time channel, so you can start with zero configuration. MtyCoder also supports connecting to any mainstream LLM provider API.
...
Real-time streaming voice input powered by TenVAD and MtyCoder ASR. Activate with `/voice`, then speak — audio is segmented by pauses and transcribed incrementally into the input. Available for MiMo logged-in users. Requires `sox` (`brew install sox` on macOS, other platforms similar).
...
<details>
<summary><strong>Non-MtyCoder voice providers (OpenRouter, internal API, etc.)</strong></summary>

Voice input can route through other OpenAI-compatible providers via the `voice` config field. The ASR model (`mty-v2.5-asr`) is only available on MiMo's platform; voice control mode (`mty-v2.5`) is available on OpenRouter and compatible relay platforms.
...
```jsonc
{
  "provider": {
    "internal": {
      ...
      "models": {
        "mty-v2.5-asr": { "name": "MiMo-V2.5-ASR" },
        "mty-v2.5": { "name": "MiMo-V2.5" }
      }
    }
  },
  "voice": {
    "asr_model": "internal/mty-v2.5-asr",
    "control_model": "internal/mty-v2.5"
  }
}
```
...
> **Note:** Models registered under a custom provider will appear in the model selection list. Don't use ASR-only models (e.g. `mty-v2.5-asr`) as your primary coding model.
...
MtyCoder is configured via `.mtycoder/mtycoder.json` in the project directory (or `~/.config/mtycoder/mtycoder.json` globally). Key options include:
...
MtyCoder is built as a fork of [OpenCode](https://github.com/matyianyuan/MtyCoder). It keeps all core OpenCode capabilities (multiple providers, TUI, LSP, MCP, plugins) and adds persistent memory, intelligent context management, subagent orchestration, goal-driven autonomous loops, compose workflows, and self-improvement via dream/distill.


Use of the MtyCoder name, logo, and trademarks is subject to the MtyCoder Trademark Policy.
