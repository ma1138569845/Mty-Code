...
<p align="center">
  <a href="https://mtycoder.example.com/zh/mtycoder">官网</a> | <a href="https://mtycoder.example.com/zh/blog/mty-code-long-horizon">博客</a>
</p>

---

MtyCoder 是一个终端原生的 AI 编程助手。它能读写代码、执行命令、管理 Git，通过持久化记忆系统，在多次会话间保持对你项目的深度理解，并自我进化。

内置 MtyCoder Auto 限时免费通道——零配置即可开始使用。也支持接入各家主流 LLM 厂商 API。
...
<details>
<summary><strong>为什么需要 <code>--legacy-peer-deps</code>？</strong></summary>

部分上游依赖存在 peer dependency 版本冲突（如 `effect` v3 vs v4、`zod` v3 vs v4）。这些只是警告，不影响安装和运行。使用 `--legacy-peer-deps` 可消除警告：

```bash
npm install -g @mty-coder/cli --legacy-peer-deps
```

此参数告诉 npm 跳过 peer dependency 验证，冲突的版本在运行时是兼容的。
</details>
...
基于 TenVAD 和 MtyCoder ASR 的实时流式语音输入。通过 `/voice` 激活，按停顿分片转写，文本逐段追加到输入框。仅对 MtyCoder 登录用户可用。需要安装 `sox`（macOS 上 `brew install sox`，其他平台类似）。
...
<details>
<summary><strong>非 MtyCoder 渠道语音输入（OpenRouter、内部 API 等）</strong></summary>

语音输入可通过 `voice` 配置字段路由到其他 OpenAI 兼容 provider。ASR 模型（`mty-v2.5-asr`）仅在 MtyCoder 平台可用；语音控制模式（`mty-v2.5`）可通过 OpenRouter 等中转平台使用。
...
> **注意**：自定义 provider 注册的模型会出现在主模型选择列表中。请勿将 ASR 专用模型（如 `mty-v2.5-asr`）用作编程主模型。
...
通过项目目录下的 `.mtycoder/mtycoder.json`（或全局 `~/.config/mtycoder/mtycoder.json`）配置。主要选项包括：
...
MtyCoder 基于 [OpenCode](https://github.com/matyianyuan/MtyCoder) fork 构建，保留其全部核心能力（多 Provider、TUI、LSP、MCP、插件），并在此基础上构建了持久化记忆、智能上下文管理、子智能体编排、目标驱动的自主循环、Compose 工作流，以及通过 dream/distill 实现的自我进化。
...
使用 MtyCoder 名称、标志和商标须遵守 MtyCoder 商标政策。
